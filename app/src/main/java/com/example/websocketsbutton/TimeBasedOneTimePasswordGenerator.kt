...
open class TimeBasedOneTimePasswordGenerator(private val secret: ByteArray, private val config: TimeBasedOneTimePasswordConfig) {
    ...
    fun counter(timestamp: Long = System.currentTimeMillis()): Long {
        ...
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            floor(timestamp.toDouble().div(TimeUnit.MILLISECONDS
                .convert(config.timeStep, config.timeStepUnit))).toLong()
        } else {
     ...
     * Convenience method for [counter].
     */
    fun counter(instant: Instant): Long = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        counter(instant.toEpochMilli())
    } else {
    ...
    fun timeslotStart(counter: Long): Long {
        val timeStepMillis = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
     ...
     * Convenience method for [generate].
     */
    fun generate(instant: Instant): String = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
     ...
     * Convenience method for [isValid].
     */
    fun isValid(code: String, instant: Instant) = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        isValid(code, instant.toEpochMilli())
    } else {
        TODO("VERSION.SDK_INT < O")
    }
